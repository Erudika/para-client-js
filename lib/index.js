...
var AWS4Signer = require('./util/AWS4Signer');
...
function ParaClient(accessKey, secretKey, options) {
	assert(accessKey && secretKey, "You have to provide your Para access and secret keys.");
	options = options || {};
	...
	function invokeSignedRequest(httpMethod, endpointURL, reqPath, headers, params, jsonEntity) {
		...
		var opts = {
			...
			method: httpMethod,
			host: host,
			path: encodeURIComponent(reqPath).replace(/%2F/g, "/"),
			headers: headers || {}
		};
...
ParaClient.prototype.update = function (obj, fn) {
	...
	return getEntity(this.invokePut(obj.getObjectURI(), obj), fn, false);
};
...
ParaClient.prototype.updateAll = function (objects, fn) {
	...
	return getEntity(this.invokePut("_batch", objects)).then(function (result) {
		var res = getItemsFromList(result);
		fn(res);
		return res;
	});
...
//var pc = new ParaClient("app:para", "6P8uwMgxnUQplN0MU0Uc0KEUgL1iy2RACZd3SLjZhGH/T9q7JxoxfA==");
