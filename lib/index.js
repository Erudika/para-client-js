...
function ParaClient(accessKey, secretKey, options) {
	...
	this.getServerVersion = function(fn) {
		fn = fn || _.noop;
		return getEntity(this.invokeGet("")).then(function (result) {
	...
	this.find = function (queryType, params, fn) {
		if (params && params instanceof Object && !_.isEmpty(params)) {
			...
			if (!params["type"]) {
				...
				return getEntity(that.invokeGet(params["type"] + "/search" + qType, params), fn);
			}
		} else {
			var res = {
				"items": [],
...
ParaClient.prototype.me = function (accessToken, fn) {
	...
	if (accessToken && _.isString(accessToken)) {
		...
		return getEntity(invokeSignedRequest("GET", this.endpoint, getFullPath("_me"), {"Authorization": auth}), fn, false);
	} else {
 ...
 * @param {type} obj the object to receive +1 votes
 * @param {type} voterid the userid of the voter
 * @param {Function} fn callback (optional)
...
ParaClient.prototype.voteUp = function (obj, voterid, fn) {
	...
	return getEntity(this.invokePatch(obj.getType() + "/" + obj.getId(), {"_voteup": voterid})).then(function (result) {
		var res = result === "true";
		fn(res);
		return res;
	});
...
ParaClient.prototype.voteDown = function (obj, voterid, fn) {
	...
	return getEntity(this.invokePatch(obj.getType() + "/" + obj.getId(), {"_votedown": voterid})).then(function (result) {
		var res = result === "true";
		fn(res);
		return res;
	});
...
ParaClient.prototype.setAppSettings = function(settings, fn) {
	...
	return getEntity(this.invokePut("_settings", settings), fn);
};
