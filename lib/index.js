...
var JWT_REFRESH_INTERVAL_SEC = 60;
var SEPARATOR = ":";
...
function ParaClient(accessKey, secretKey, options) {
	assert(secretKey && secretKey.length > 6, "Secret key appears to be invalid. Make sure you call 'signIn()' first.");
	options = options || {};
	...
	function invokeSignedRequest(httpMethod, endpointURL, reqPath, headers, params, jsonEntity) {
		if (!accessKey || (!secretKey && !that.tokenKey)) {
			throw "Security credentials are invalid.";
		}
		...
		if (that.tokenKey !== null) {
			...
			AWS4Signer.sign(opts, {accessKeyId: accessKey, secretAccessKey: secretKey});
		}
...
ParaClient.prototype.newKeys = function (fn) {
	return getEntity(this.invokePost("_newkeys"), fn);
};
 ...
 * @param {String} resourceName resource name or object type
 * @param {Array} permission a set of HTTP methods
 * @param {Function} fn callback (optional)
 * @returns {RSVP.Promise} a map of the permissions for this subject id
 */
ParaClient.prototype.grantResourcePermission = function(subjectid, resourceName, permission, fn) {
	fn = fn || _.noop;
	if (!subjectid || !resourceName || !permission || !_.isArray(permission)) {
		fn({});
		return resolve({});
	}
	return getEntity(this.invokePut("_permissions/" + subjectid + "/" + resourceName, permission), fn);
};
 ...
 * @returns {RSVP.Promise} a map of the permissions for this subject id
 */
ParaClient.prototype.revokeResourcePermission = function(subjectid, resourceName, fn) {
	fn = fn || _.noop;
	if (!subjectid || !resourceName) {
		fn({});
		return resolve({});
	}
	return getEntity(this.invokeDelete("_permissions/" + subjectid + "/" + resourceName), fn);
};
 ...
 * @param {String} resourceName resource name (type)
 * @param {String} httpMethod HTTP method name
 * @param {Function} fn callback (optional)
 * @returns {RSVP.Promise} true if allowed
 */
ParaClient.prototype.isAllowedTo = function(subjectid, resourceName, httpMethod, fn) {
	fn = fn || _.noop;
	if (!subjectid || !resourceName || !httpMethod) {
		fn(false);
		return resolve(false);
	}
	var url = "_permissions/" + subjectid + "/" + resourceName + "/" + httpMethod;
	return getEntity(this.invokeGet(url)).then(function (result) {
		var res = result === "true";
		fn(res);
		return res;
...
ParaClient.prototype.refreshToken = function(fn) {
	...
	var interval = (JWT_REFRESH_INTERVAL_SEC * 1000);
	var notExpired = that.tokenKeyExpires !== null && that.tokenKeyExpires > now;
	var canRefresh = that.tokenKeyLastRefresh !== null && (
			(that.tokenKeyLastRefresh + interval) < now ||
			(that.tokenKeyLastRefresh + interval) > that.tokenKeyExpires);
	...
	if (that.tokenKey !== null && notExpired && canRefresh) {
		return getEntity(this.invokeGet(JWT_PATH)).then(function (result) {
			if (result !== null && result["user"] && result["jwt"]) {
				...
				that.tokenKey = jwtData["access_token"];
				that.tokenKeyExpires = jwtData["expires"];
				that.tokenKeyLastRefresh = new Date().getTime();
				fn(true);
				return true;
			} else {
				clearAccessToken();
